<?php
   if($_REQUEST["code"]){
	  ...
	  // echo $t;
	   include("enter.php");
	   $x=1;
		$rs=mysql_query("select * from moviereview where typess='$g'");
		while($r=mysql_fetch_array($rs)){
		$x=$x+1;
		}
		$x=$x-1;
	   
	   $rs=mysql_query("select * from moviereview where typess='$g' order by sn DESC");
	   while($r=mysql_fetch_array($rs)){
	...
	?>                                                       
		    ...
		    <h6><a href="full-story-movie.php?id=<?=$r[1]?>"><?=$r[1]?></a></h6></div>
